...
const body = stripRoot(expression.trim());
...
for (const step of splitSteps(body)) {
  ...
  if (name.endsWith("[x]") || bracket === "x") {
    ...
    resolvable = false;
    caveats.push(`'${base}[x]' is a choice element; the concrete property (e.g. ${base}Quantity, ${base}String) depends on the instance.`);
    segments.push({ name: base, kind: "choice", pointerPart: null });
    continue;
  }
