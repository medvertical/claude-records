...
| `processing` | The validator could not process input, terminology, packages, references, or expressions. | Fix setup first; avoid resource edits until the processing cause is known. | Needed only after setup is ruled out. | Very likely. Check package cache, validator version, network/tx server, and IG build artifacts. |
