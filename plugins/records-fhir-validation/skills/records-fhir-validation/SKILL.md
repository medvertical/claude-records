...
2. **Hosted API mode**: If `RECORDS_API_URL` is set, use the Records API. Include `Authorization: Bearer $RECORDS_API_KEY` only when `RECORDS_API_KEY` is set. Do not invent endpoints; inspect local docs or API code when working inside the Records repo.
...
```bash
records validate-file <file-or-directory> --server "$RECORDS_API_URL"
```
...
- name: Validate FHIR resources with Records
  ...
  run: records validate-file ./fhir-resources --server "$RECORDS_API_URL" --format junit
```
